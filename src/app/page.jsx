...
export default function LoginPage() {
  ...
  const fallbackRankingContract = useMemo(
    () => mockManifest?.surfaceFallbackRankingContracts?.find((item) => item.screenId === 'login') ?? null,
    [mockManifest]
  );
        ...
        <section className="flex flex-col justify-between rounded-[2rem] border border-white/10 bg-black/30 p-8 lg:p-10">
          ...
          {fallbackRankingContract ? (
            <div className="mt-6 rounded-[1.6rem] border border-white/10 bg-black/20 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.25em] text-slate-400">{fallbackRankingContract.title}</p>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">{fallbackRankingContract.summary}</p>
                </div>
                <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.22em] text-slate-300">
                  Leader: {healthiestConnection?.name ?? activeConnection?.name ?? 'Current draft'}
                </span>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
