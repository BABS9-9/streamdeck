          ...
          <article key={item.label} className={`rounded-2xl border p-5 ${toneClasses[item.tone]}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {item.owner ? (
                  <>
                    <span className={`rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-[0.2em] ${connectionStatusTone[item.owner.status]}`}>
                      {item.owner.status}
                    </span>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Runtime stability status</p>
                <p className="mt-2 text-sm leading-6 text-white">{item.stabilityStatus}</p>
              </div>
              <div>
