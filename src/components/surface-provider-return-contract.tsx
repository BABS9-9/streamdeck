...
const toneLabels = {
  ready: 'Return is earned',
  watch: 'Return needs proof',
  recover: 'Stay on rescue',
} as const;

type SurfaceProviderReturnContractProps = {
  contract: MockProviderManifest['surfaceProviderReturnContracts'][number] | null;
  badge?: string;
};

export function SurfaceProviderReturnContract({
  contract,
  badge = 'Provider-return truth',
}: SurfaceProviderReturnContractProps) {
        ...
        {contract.returns.map((item) => (
          <article key={item.label} className={`rounded-2xl border p-5 ${toneClasses[item.tone]}`}>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Return trigger</p>
                <p className="mt-2 text-sm leading-6 text-white">{item.returnTrigger}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Stay on rescue trigger</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.stayOnRescueTrigger}</p>
              </div>
            </div>
          </article>
        ))}
