...
export function LiveBrowser() {
  ...
  const evidenceLedger = useMemo(
    () => mockManifest?.surfaceEvidenceLedgers?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const freshnessBoard = useMemo(
    () => mockManifest?.surfaceFreshnessBoards?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const contradictionBoard = useMemo(
    () => mockManifest?.surfaceContradictionBoards?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  ...
  const autonomyBoundary = useMemo(
    () => mockManifest?.surfaceAutonomyBoundaries?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const identityAnchor = useMemo(
    () => mockManifest?.surfaceIdentityAnchors?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const confidenceFloor = useMemo(
    () => mockManifest?.surfaceConfidenceFloors?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const recoveryWitness = useMemo(
    () => mockManifest?.surfaceRecoveryWitnesses?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const fallbackCost = useMemo(
    () => mockManifest?.surfaceFallbackCosts?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const rescueReceipt = useMemo(
    () => mockManifest?.surfaceRescueReceipts?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const proofDebt = useMemo(
    () => mockManifest?.surfaceProofDebts?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const claimCeiling = useMemo(
    () => mockManifest?.surfaceClaimCeilings?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const interruptionBudget = useMemo(
    () => mockManifest?.surfaceInterruptionBudgets?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const retryContract = useMemo(
    () => mockManifest?.surfaceRetryContracts?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
  const providerSwitchContract = useMemo(
    () => mockManifest?.surfaceProviderSwitchContracts?.find((item) => item.screenId === 'live') ?? null,
    [mockManifest]
  );
      ...
      <section className="space-y-6">
          ...
          {mockHealth ? (
            <>
              ...
              <MockOperationsConsole
                ...
                screenId="live"
                title="Live rehearsal operations"
                intro="Live now shares the same adapter-fed rehearsal console as Login and Home, so surf, preview, and recovery checks stay synchronized."
                scenario={scenario}
        ...
        {recoveryWitness ? (
          <div className="rounded-[1.5rem] border border-white/10 bg-black/20 px-5 py-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-white/80">
                Selected-card proof stays visible
              </span>
            </div>
            <div className="mt-4 grid gap-3 xl:grid-cols-2">
        ...
        {providerStabilityContract ? (
          <div className="rounded-[1.5rem] border border-white/10 bg-black/20 px-5 py-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">{providerStabilityContract.title}</p>
                <p className="mt-2 text-sm text-slate-300">{providerStabilityContract.summary}</p>
              </div>
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-white/80">
                Surf stability honesty visible
              </span>
            </div>
            <div className="mt-4 grid gap-3 xl:grid-cols-2">
