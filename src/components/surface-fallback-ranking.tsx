...
const toneLabels = {
  ready: 'Best exact rescue',
  watch: 'Approximate leader',
  recover: 'Hard recovery',
} as const;

type SurfaceFallbackRankingProps = {
  runtime: SurfaceFallbackRankingRuntimeContract | null;
  badge?: string;
};

export function SurfaceFallbackRanking({
  runtime,
  badge = 'Fallback ranking',
}: SurfaceFallbackRankingProps) {
    ...
    <section className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-full border border-white/10 bg-black/20 px-3 py-2 text-[10px] uppercase tracking-[0.22em] text-white/80">
          Rescue order visible
        </span>
      </div>

      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        {runtime.rankings.map((item) => (
          <article key={item.label} className={`rounded-2xl border p-5 ${toneClasses[item.tone]}`}>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Current leader</p>
                <p className="mt-2 text-sm leading-6 text-white">{item.currentLeader}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Ranking evidence</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.rankingEvidence}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Rerank trigger</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.rerankTrigger}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Rescue order</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.rescueOrder}</p>
              </div>
              <div>
