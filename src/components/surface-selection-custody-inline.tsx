...
export function SurfaceSelectionCustodyInline({
  manifest,
  screenId,
  runtime,
}: {
  ...
  screenId: ScreenId;
  runtime: SelectionCustodyRuntime;
}) {
  const contract = manifest?.surfaceSelectionCustodyContracts.find((item) => item.screenId === screenId) ?? null;
    ...
    <section className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <article className="rounded-[1.35rem] border border-white/10 bg-black/20 p-4">
          <p className="text-[11px] uppercase tracking-[0.22em] text-slate-500">Active subject</p>
          <p className="mt-3 text-sm font-medium text-white">{runtime.activeSubject}</p>
        </article>
        <article className="rounded-[1.35rem] border border-white/10 bg-black/20 p-4">
      ...
      <div className="mt-5 grid gap-3 xl:grid-cols-3">
        {contract.custody.map((item) => (
          <article key={item.label} className="rounded-[1.35rem] border border-white/10 bg-black/20 p-4">
            ...
            <p className="mt-3 text-[11px] uppercase tracking-[0.22em] text-slate-500">Subject</p>
            <p className="mt-2 text-sm leading-6 text-slate-200">{item.subject}</p>
