...
const toneLabels = {
  ready: 'Still premium',
  watch: 'Budget thinning',
  recover: 'Recovery should lead',
} as const;

type SurfaceInterruptionBudgetProps = {
  contract: MockProviderManifest['surfaceInterruptionBudgets'][number] | null;
  badge?: string;
};

export function SurfaceInterruptionBudget({
  contract,
  badge = 'Interruption budget',
}: SurfaceInterruptionBudgetProps) {
        ...
        {contract.budgets.map((item) => (
          <article key={item.label} className={`rounded-2xl border p-5 ${toneClasses[item.tone]}`}>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Acceptable delay</p>
                <p className="mt-2 text-sm leading-6 text-white">{item.acceptableDelay}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Continuity layer</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.continuityLayer}</p>
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-white/70">Escalation trigger</p>
                <p className="mt-2 text-sm leading-6 text-white/85">{item.escalationTrigger}</p>
              </div>
            </div>
          </article>
        ))}
