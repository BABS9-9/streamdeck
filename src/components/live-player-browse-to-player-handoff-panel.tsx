...
export function LivePlayerBrowseToPlayerHandoffPanel({
  ...
  return (
    <section className={`rounded-[1.75rem] border p-6 ${toneStyles[contract.tone]}`}>
      ...
      <div className="mt-5">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[11px] uppercase tracking-[0.22em] text-slate-400">Authority ledger</p>
          <span className="text-[10px] uppercase tracking-[0.2em] text-white/55">Who wins each truth domain</span>
        </div>
        <p className="mt-3 max-w-3xl text-xs leading-5 text-white/65">
          This matrix gives the dock one backend-owned answer for which surface currently owns launch provenance, provider truth, line pressure, continuity, transfer disclosure, and recovery routing.
        </p>
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          {contract.authorityLedger.map((entry) => (
            <article key={entry.id} className={`rounded-[1.35rem] border p-4 ${toneStyles[entry.tone]}`}>
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-medium text-white">{entry.label}</p>
                <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1 text-[10px] uppercase tracking-[0.2em] text-white/80">
              ...
              <div className="mt-4 space-y-3 text-xs leading-5 text-white/80">
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Active owner</p>
                  <p className="mt-1">{entry.activeOwner}</p>
                </div>
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Winning surface</p>
                  <p className="mt-1">{entry.winningSurface}</p>
                </div>
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Hold rule</p>
                  <p className="mt-1">{entry.holdRule}</p>
                </div>
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Handoff trigger</p>
                  <p className="mt-1">{entry.handoffTrigger}</p>
                </div>
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Visible fallback</p>
                  <p className="mt-1">{entry.visibleFallback}</p>
                </div>
                <div>
                  <p className="uppercase tracking-[0.18em] text-white/55">Affected surfaces</p>
                  <p className="mt-1">{entry.affectedSurfaces.join(', ')}</p>
