...
export function HomeDashboard() {
  ...
  const providerChoiceContract = useMemo(
    () => mockManifest?.surfaceProviderChoiceContracts?.find((item) => item.screenId === 'home') ?? null,
    [mockManifest]
  );
  const providerReturnContract = useMemo(
    () => mockManifest?.surfaceProviderReturnContracts?.find((item) => item.screenId === 'home') ?? null,
    [mockManifest]
  );
  const providerStabilityContract = useMemo(
    () => mockManifest?.surfaceProviderStabilityContracts?.find((item) => item.screenId === 'home') ?? null,
    [mockManifest]
  );
  const canonicalProviderIdentityContract = useMemo(
    () => mockManifest?.surfaceCanonicalProviderIdentityContracts?.find((item) => item.screenId === 'home') ?? null,
    [mockManifest]
  );
  const fallbackRankingContract = useMemo(
    () => mockManifest?.surfaceFallbackRankingContracts?.find((item) => item.screenId === 'home') ?? null,
    [mockManifest]
  );
      ...
      {canonicalProviderIdentityContract ? (
        <section className="rounded-[1.5rem] border border-white/10 bg-black/20 px-5 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-white/80">
              Canonical owner in browse
            </span>
          </div>
          {activeCanonicalIdentity ? (
            <div className="mt-4 rounded-2xl border border-violet-400/20 bg-violet-500/10 p-4">
              <p className="text-[11px] uppercase tracking-[0.22em] text-violet-200">Active provider owner</p>
              <p className="mt-2 break-all text-sm font-semibold text-white">{activeCanonicalIdentity}</p>
              <p className="mt-2 text-sm text-violet-100/80">{activeIdentityAliases} alias form{activeIdentityAliases === 1 ? '' : 's'} currently resolve to this owner.</p>
            </div>
          ) : null}
          <div className="mt-4 grid gap-3 xl:grid-cols-2">
      ...
      {providerStabilityContract ? (
        <section className="rounded-[1.5rem] border border-white/10 bg-black/20 px-5 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-white/80">
              Browse stability honesty visible
            </span>
          </div>
          <div className="mt-4 grid gap-3 xl:grid-cols-2">
